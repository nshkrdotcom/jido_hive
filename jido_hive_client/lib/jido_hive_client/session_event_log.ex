defmodule JidoHiveClient.SessionEventLog do
  @moduledoc false

  @default_limit 200
  @schema_version "jido_hive/session_event.v1"
  ...
  def append(%__MODULE__{} = log, attrs) when is_map(attrs) do
    entry = %{
      event_id: Map.get(attrs, :event_id, "session-event-#{log.next_sequence}"),
      source: Map.get(attrs, :source, :session),
      topic: Map.get(attrs, :topic, "room.session"),
      type: normalize_type(Map.get(attrs, :type, "session.changed")),
  ...
  defp normalize_type(_other), do: "session.changed"
