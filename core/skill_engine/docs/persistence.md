# Jido Hive Skill Engine Persistence

## Scope

Jido Hive Skill Engine owns memory-default governed agent or memory contract owner. This document is package-local and is the persistence contract for `core/skill_engine` in `jido_hive`.
...
```bash
cd core/skill_engine && mix test; root mix ci
```
