# Jido Hive Agent Coordinator Persistence

## Scope

Jido Hive Agent Coordinator owns memory-default governed agent or memory contract owner. This document is package-local and is the persistence contract for `core/agent_coordinator` in `jido_hive`.
...
```bash
cd core/agent_coordinator && mix test; root mix ci
```
