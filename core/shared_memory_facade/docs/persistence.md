# Jido Hive Shared Memory Facade Persistence

## Scope

Jido Hive Shared Memory Facade owns memory-default governed agent or memory contract owner. This document is package-local and is the persistence contract for `core/shared_memory_facade` in `jido_hive`.
...
Memory/ref-only records by default.
...
```bash
cd core/shared_memory_facade && mix test; root mix ci
```
...
Valid durability claims require explicit profile selection, adapter capability, migration or substrate preflight, redacted evidence, focused tests, repo QC, and a pushed commit. Durable adapter refs require explicit durable adapter and preflight proof.
...
No default durable schema ownership.
