# Jido Hive Workspace Persistence

## Scope

Jido Hive Workspace owns human-plus-AI collaboration product with explicit server, client, surface, publication, and worker ownership. This document is package-local and is the persistence contract for `.` in `jido_hive`.
...
Product-facing surfaces are no-bypass scanned. jido_hive_server and jido_hive_publications are local store-owner packages.
...
Valid durability claims require explicit profile selection, adapter capability, migration or substrate preflight, redacted evidence, focused tests, repo QC, and a pushed commit. Local SQLite/Ecto persistence is owned by jido_hive_server and publications; governed lower connector state uses explicit Jido Integration owner paths.
...
jido_hive_server owns room persistence migrations. Publications use the server repo and must document that owner link.
