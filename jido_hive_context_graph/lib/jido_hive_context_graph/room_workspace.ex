defmodule JidoHiveContextGraph.RoomWorkspace do
  @moduledoc """
  Structured room workspace data for headless clients and terminal frontends.
  """

  alias JidoHiveContextGraph.{RoomInsight, RoomWorkflow}
  ...
  def build(snapshot, opts) when is_map(snapshot) do
    ...
    %{
      room_id: Map.get(snapshot, "id"),
      status: Map.get(snapshot, "status", "unknown"),
      objective: Map.get(snapshot, "name") || control_plane.objective,
  ...
  defp normalize_conversation_entry(entry) do
    %{
      ...
      kind: conversation_kind(entry) || "chat",
  ...
  defp message_object_entry(object) do
    %{
      ...
      "kind" => "chat",
      "payload" => %{"summary" => body(object) || title(object) || ""}
    }
  end
  ...
  defp conversation_body(entry) do
    entry
    |> conversation_body_candidates()
    |> first_present_conversation_body()
  end

  defp conversation_kind(entry) do
    map_value(entry, "kind")
  end

  defp conversation_body_candidates(entry) do
    payload = Map.get(entry, "payload") || Map.get(entry, :payload) || %{}
  ...
  defp first_present_conversation_body(candidates) when is_list(candidates) do
