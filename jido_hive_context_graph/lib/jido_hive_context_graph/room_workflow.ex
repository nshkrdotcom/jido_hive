defmodule JidoHiveContextGraph.RoomWorkflow do
  @moduledoc """
  ...
  def inspect_sync(%{
    ...
    %{
      room_id: value(room_snapshot, "id"),
  ...
  defp default_summary(snapshot) do
    %{
      ...
      blockers: [],
      publish_ready: false,
      publish_blockers: ["Server workflow summary unavailable"],
      graph_counts: %{
  ...
  defp snapshot_objective(snapshot) do
    value(snapshot, "name") || "No room objective available"
  end

  defp summary_publish_ready(workflow_summary, _snapshot) do
