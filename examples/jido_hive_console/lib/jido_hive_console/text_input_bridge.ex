defmodule JidoHiveConsole.TextInputBridge do
  ...
  def sync(%Model{} = state) do
    state
    |> sync_ref(:room_input_ref, state.input_buffer)
    |> sync_ref(:conflict_input_ref, state.conflict_input_buf)
    |> sync_ref(:wizard_brief_input_ref, Map.get(state.wizard_fields, "brief", ""))
    |> sync_publish_ref()
  end

  @spec handle_room_key(Model.t(), String.t()) :: Model.t()
  def handle_room_key(%Model{} = state, code) when is_binary(code) do
    %{state | input_buffer: edit_or_fallback(state.room_input_ref, state.input_buffer, code)}
  end
