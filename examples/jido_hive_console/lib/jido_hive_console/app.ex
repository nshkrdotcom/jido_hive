defmodule JidoHiveConsole.App do
  ...
  def update(:status_animation_tick, state) do
  ...
  def update({:event_log_update, entries, cursor}, state) do
    next_state =
      ...
      |> then(fn refreshed_state ->
        formatted = entries |> Enum.map(&Projection.format_event_entry/1) |> Enum.reverse()
        lines = (formatted ++ refreshed_state.event_log_lines) |> Enum.uniq() |> Enum.take(200)
        %{refreshed_state | event_log_lines: lines, event_log_cursor: cursor}
      end)
  ...
  defp wizard_submit_workers(state) do
    ...
    cond do
      ...
      state.wizard_available_targets == [] ->
        {Model.set_status(state, "No worker targets online yet; auto-refreshing", :warn), []}
  ...
  defp maybe_set_empty_target_warning(state, []),
    do: Model.set_status(state, "No worker targets online yet; auto-refreshing", :warn)
  ...
  defp queue_room_chat_submit(state, text) do
    ...
    {next_state,
     [
       ...
       {:timer, 125, :status_animation_tick}
     ]}
  end
