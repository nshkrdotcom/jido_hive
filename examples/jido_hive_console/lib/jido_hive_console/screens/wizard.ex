defmodule JidoHiveConsole.Screens.Wizard do
  @moduledoc false
  ...
  alias JidoHiveConsole.{HelpGuide, InputKey, Model, ScreenUI}
  ...
  defp body_widget(%Model{wizard_step: 3, wizard_targets_state: status} = state)
       when status in [:idle, :loading] do
    ScreenUI.pane(
      worker_title(state),
      [
        "#{waiting_indicator()} Waiting for worker targets...",
        "",
        "Detected workers: #{length(state.wizard_available_targets)}",
        "Auto-refreshing while workers connect.",
        "Start local workers with bin/hive-clients if needed."
      ],
      border_fg: :cyan
    )
  end
  ...
  defp body_widget(%Model{wizard_step: 3, wizard_available_targets: []} = state) do
    ScreenUI.pane(
      worker_title(state),
      [
        "#{waiting_indicator()} No worker targets online yet.",
        "",
        "Detected workers: 0",
        "Auto-refreshing while workers connect.",
        "Room creation requires at least one registered worker target.",
        "Start local workers with bin/hive-clients and wait for them to reach ready."
      ],
      border_fg: :yellow
    )
  end
  ...
  defp waiting_indicator do
    case rem(div(System.monotonic_time(:millisecond), 250), 4) do
      0 -> "|"
      1 -> "/"
      2 -> "-"
      _other -> "\\"
