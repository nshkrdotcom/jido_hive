defmodule JidoHiveConsole.AppTest do
  ...
  defmodule EmbeddedStub do
    ...
    defp test_pid do
      Application.fetch_env!(:jido_hive_console, :app_test_pid)
    end
  end
  ...
  setup do
    Application.put_env(:jido_hive_console, :app_test_pid, self())

    on_exit(fn ->
      Application.delete_env(:jido_hive_console, :app_test_pid)
    end)
  ...
  test "room submit result keeps the local pending echo until the message becomes visible", %{
    model: model
  } do
    state =
      %{
  ...
  test "event log updates refresh the room snapshot for room screens", %{
    ...
    Agent.update(embedded, fn state ->
      Map.put(state, :snapshot, %{
        "room_id" => "room-1",
        "status" => "running",
        "dispatch_state" => %{"completed_slots" => 1, "total_slots" => 2},
        "participants" => [],
        "timeline" => [%{"kind" => "contribution.recorded", "event_id" => "evt-1"}],
        "context_objects" => [
          %{
            "context_id" => "ctx-message-1",
            "object_type" => "message",
            "title" => "alice said",
            "body" => "hello from timeline refresh",
            "authored_by" => %{"participant_id" => "alice"}
          }
        ],
        "contributions" => [
  ...
  test "wizard view distinguishes empty worker list from loading" do
    ...
    assert loading_text =~ "Waiting for worker targets..."
    assert loading_text =~ "Select Workers (0 online)"
    assert loading_text =~ "Detected workers: 0"
    assert ready_text =~ "No worker targets online yet."
    assert ready_text =~ "Auto-refreshing while workers connect."
  ...
  test "wizard submit workers reports no available targets" do
    ...
    assert next_state.status_line == "No worker targets online yet; auto-refreshing"
  ...
  test "wizard entering worker selection triggers a fresh target fetch" do
    model =
      Model.new([])
      |> Map.put(:active_screen, :wizard)
      |> Map.put(:wizard_step, 2)
    ...
    {next_state, effects} = App.update(:wizard_enter, model)

    assert next_state.wizard_step == 3
    assert next_state.wizard_cursor == 0
    assert next_state.wizard_targets_state == :loading
    assert effects == [{:timer, 0, :fetch_wizard_targets}]
  end

  test "poll refreshes worker targets while the wizard is on select workers" do
    model =
      Model.new([])
      |> Map.put(:active_screen, :wizard)
      |> Map.put(:wizard_step, 3)
    ...
    {next_state, effects} = App.update(:poll, model)

    assert next_state == model
    assert {:timer, model.poll_interval_ms, :poll} in effects
    assert {:timer, 0, :fetch_wizard_targets} in effects
  end
  ...
  test "wizard create room runs asynchronously and keeps the app responsive" do
    model =
      ...
      |> Map.put(:wizard_fields, %{
        "brief" => "Develop specs for debugging console",
