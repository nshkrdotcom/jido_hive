...
- `jido_hive_server`
  authoritative room truth, relay, persistence, and connector state
- `jido_hive_client`
...
- `jido_hive_publications`
  explicit publication planning, execution, and publication UI models
...
- the shared surface decides how reusable room workflows are exposed to UI
  packages
- explicit extensions such as `jido_hive_publications` layer optional behavior
  on top of canonical room resources
...
- room creation, execution, and persistence
...
`jido_hive_publications` owns:

- publication planning and execution
- publication persistence through the shared server repo
- publication workspace display models
- publication LiveView mounts consumed by downstream surfaces
