...
- `jido_hive_server`
  authoritative room truth, room-event truth, reduction, and connector state
- `jido_hive_client`
  reusable operator workflows, room-scoped local session behavior, headless CLI
- `jido_hive_surface`
  shared room workflows for TUI and web packages
...
- does the server think the room is `waiting`, `active`, `completed`, `closed`,
  or `failed`?
