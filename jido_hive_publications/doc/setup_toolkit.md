...
Publication planning and execution are no longer exposed through `setup/hive`.
Use the explicit `jido_hive_publications` extension through the web route
`/rooms/:room_id/publications` or the downstream surfaces that mount that
extension.
